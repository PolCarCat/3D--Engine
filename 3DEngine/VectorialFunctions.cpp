...
const float2 VectorialFunctions::rotatef2(const float2 &u, float angle) 
{
...
const float3 VectorialFunctions::rotatef3(const float3 &u, float angle, const float3 &v)
{
	return *(float3*)&(rotatef4x4(angle, v) * float4(u, 1.0f));
}

const float4x4 VectorialFunctions::rotatef4x4(float angle, const float3 &u) 
{
	float4x4 Rotate;
...
const float4x4 VectorialFunctions::perspective(float fovy, float aspect, float n, float f)
{
...
const float4x4 VectorialFunctions::ortho(float left, float right, float bottom, float top, float n, float f)
{
	float4x4 Ortho;
